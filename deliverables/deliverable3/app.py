...
Author: Daniel Bai
Date: 2025
Description: Enhanced Flask web application with advanced queries and complex reporting features
"""

from flask import Flask, render_template, request, redirect, url_for, flash
from database import (
    create_volunteer, get_all_volunteers, get_volunteer_by_id, update_volunteer_phone,
    get_all_organisations, get_organisation_by_id, get_all_events, get_event_by_id, delete_event,
    validate_email, validate_phone, test_database_connection,
    search_volunteers_by_skill, get_volunteers_for_organisation_events,
    get_event_statistics, get_volunteers_with_calculated_fields,
    get_skill_distribution, get_organisation_event_summary, get_all_skills
)
...
# ====================================
# ADVANCED QUERY ROUTES (WEEK 3)
# ====================================

@app.route('/volunteers/search', methods=['GET', 'POST'])
def search_volunteers():
    ...
    if request.method == 'POST':
        search_skill = request.form.get('skill_name', '').strip()
        
        if search_skill:
            volunteers = search_volunteers_by_skill(search_skill)
            if volunteers is None:
                flash('Error searching for volunteers', 'error')
                volunteers = []
            elif len(volunteers) == 0:
                flash(f'No volunteers found with skill: {search_skill}', 'info')
        else:
            flash('Please enter a skill to search for', 'error')
    
    return render_template('search_volunteers.html', 
                         volunteers=volunteers, 
                         skills=skills,
                         search_skill=search_skill)

@app.route('/organisations/<int:org_id>/volunteers')
def organisation_volunteers(org_id):
    """
    Multi-table JOIN query: Show all volunteers for organisation events
    """
    volunteers = get_volunteers_for_organisation_events(org_id)
    organisation = get_organisation_by_id(org_id)
    ...
    if volunteers is None:
        flash('Error retrieving volunteer information', 'error')
        volunteers = []
    
    return render_template('organisation_volunteers.html', 
                         volunteers=volunteers,
...
def statistics():
    """
    Aggregate statistics page with COUNT and GROUP BY
    """
    event_stats = get_event_statistics()
    skill_distribution = get_skill_distribution()
    org_summary = get_organisation_event_summary()
    
    if event_stats is None:
        event_stats = []
        flash('Error retrieving event statistics', 'error')
    
    if skill_distribution is None:
        skill_distribution = []
        flash('Error retrieving skill distribution', 'error')
        
    if org_summary is None:
        org_summary = []
        flash('Error retrieving organisation summary', 'error')
...
@app.route('/volunteers/directory')
def volunteer_directory():
    """
    Calculated fields: Display volunteers with concatenated names and calculated age
    """
    volunteers = get_volunteers_with_calculated_fields()
    
    if volunteers is None:
        flash('Error retrieving volunteer directory', 'error')
        volunteers = []
    
    return render_template('volunteer_directory.html', volunteers=volunteers)
